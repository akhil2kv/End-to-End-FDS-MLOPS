...
def generate_customer_profiles_table(n_customers, random_state=0):
    ...
    return pd.DataFrame(customer_id_properties, columns=['CUSTOMER_ID', 'x_customer_id', 'y_customer_id', 'mean_amount', 'std_amount', 'mean_nb_tx_per_day'])
...
def add_frauds(customer_profiles_table, terminal_profiles_table, transactions_df):
    ...
    print(f"Number of frauds from scenario 1: {str(nb_frauds_scenario_1)}")
    ...
    print(f"Number of frauds from scenario 2: {str(nb_frauds_scenario_2)}")
    ...
    for day in range(transactions_df.TX_TIME_DAYS.max()):
        ...
        random.seed(day)
        index_fauds = random.sample(list(compromised_transactions.index.values), k=nb_compromised_transactions // 3)
    ...
    print(f"Number of frauds from scenario 3: {str(nb_frauds_scenario_3)}")
